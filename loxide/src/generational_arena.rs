/*!
----------
This is a modified version of https://github.com/fitzgen/generational-arena/.

Here we change the Index to be 8 bytes instead of 16 bytes by making the index and the generation counter both u32s.

We also use a NonZeroU64 and bitwise-or the the idx and generation so we can make use of the [null-pointer optimization](https://doc.rust-lang.org/std/option/index.html#representation).
...
impl Index {
    ...
    pub fn from_raw_parts(index: u32, generation: u32) -> Index {
        let l = (index as u64) << 4;
        Index(NonZeroU64::new(l | generation as u64).unwrap())
    }
    ...
    /// Get the index part of this `Index`.
    #[inline]
    pub fn index(self) -> u32 {
        (self.0.get() >> 4) as u32
    }

    /// Get the generation part of this `Index`.
    #[inline]
...
impl<T> Arena<T> {
    ...
    pub fn get2_mut(&mut self, i1: Index, i2: Index) -> (Option<&mut T>, Option<&mut T>) {
        ...
        let (raw_item1, raw_item2) = {
            let (xs, ys) = self
                .items
                .split_at_mut(cmp::max(i1.index() as usize, i2.index() as usize));
            if i1.index() < i2.index() {
        ...
        loop {
            match self.inner.next() {
                ...
                Some((
                    ...
                    self.len -= 1;
                    let idx = Index::from_raw_parts(index as u32, generation);
